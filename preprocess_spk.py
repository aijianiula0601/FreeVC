...
def build_from_path(in_dir, out_dir, weights_fpath, num_workers=1):
    ...
    if spk_encoder is None:
        spk_encoder = SpeakerEncoder(weights_fpath)

    # for wav_path in wavfile_paths:
    #     futures.append(executor.submit(
    #         partial(_compute_spkEmbed, out_dir, wav_path, spk_encoder)))
    # return [future.result() for future in tqdm(futures)]
    for wav_path in tqdm(wavfile_paths):
        _compute_spkEmbed(out_dir, wav_path, spk_encoder)
...
if __name__ == "__main__":
    ...
    spk_embed_out_dir = args.out_dir_root
